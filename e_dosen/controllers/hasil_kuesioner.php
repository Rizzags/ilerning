...
class Hasil_kuesioner extends CI_Controller
{
  ...
  public function index()
  {
		...
		if( $this->tipe_user != '1'  && $this->tipe_user != '4' ){
          ...
          //isi konten
          $data['isicontent'] = 'e_dosen/hasil_kuesioner/_hasil_kuesioner';
  ...
  public function get_mata_kuliah_kues()
  {
		...
		foreach ($y->result_array() as $row) {
            $tabel.='<tr>
					...
					<td><a target="_blank" href="'.base_url().'e_dosen/hasil_kuesioner/rincian_data_kuesioner/'.my_number_encrypt($row['id_jadual']).' ">cetak</a></td>
					</tr>';
