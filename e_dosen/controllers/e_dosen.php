...
/*-----------------------------------------------------------------------------------*/
	...
	function status_kuesioner_dosen()
	{
		if (!$this->tank_auth->is_logged_in()) {
		  ...
		  //cek tipe user
		  if(( $this->tipe_user == '2' ) || ( $this->tipe_user == '3' )) {
			  redirect('/e_dosen');
		  }
  ...
  public function status_kunci_tabel_nilai()
  {
    if (!$this->tank_auth->is_logged_in()) {
      ...
      $crud->set_theme('datatables');
	  //$crud->set_theme('flexigrid');
//Jika yang dilihat adalah periode saat ini 
/*
...
INNER JOIN krs B ON A.id_jadual=B.id_jadual) A
INNER JOIN jadual B ON A.id_jadual=B.id_jadual 
...
/*Jika yang dilihat adalah periode sebelumnya
*/
...
INNER JOIN krs B ON A.id_jadual=B.id_jadual) A
INNER JOIN jadual B ON A.id_jadual=B.id_jadual 
WHERE A.thn_akademik=(SELECT thn_akademik FROM kalender_akademik ORDER BY thn_akademik DESC, periode DESC LIMIT 1,1)
AND A.periode= (SELECT periode FROM kalender_akademik ORDER BY thn_akademik DESC, periode DESC LIMIT 1,1)
) A
INNER JOIN dosen B
ON A.id_dosen=B.id_dosen
ORDER BY B.nama ASC, A.id_status DESC"); //Query text here
